...
import { duplicarProposta, excluirProposta } from "@/app/actions/propostas";
  ...
  const handleDuplicate = () => {
    ...
    startTransition(async () => {
      ...
      if (res.error) {
        ...
        router.push(`/admin/propostas/${res.id}/construtor`);
      }
    });
  };
  ...
  return (
    <div className="relative inline-block text-left" ref={menuRef}>
      <button
        ...
        disabled={isPending}
        className="flex items-center justify-center w-8 h-8 rounded-full text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:opacity-50"
      >
        {isPending ? (
          ...
          <div className="py-1">
            ...
            {podeEditar && (
              <button
                ...
                disabled={isPending}
                className="flex w-full items-center px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-left disabled:opacity-50"
              >
                <svg className="mr-3 h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {podeEditar && status === "rascunho" && (
              <button
                ...
                disabled={isPending}
                className="flex w-full items-center px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-left disabled:opacity-50"
              >
