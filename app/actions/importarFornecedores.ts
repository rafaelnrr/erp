...
export async function importarPlanilhaFornecedores(formData: FormData): Promise<ResultadoImportacao> {
  ...
  const { data: existentes } = await supabase.from("fornecedores").select("id, nome");
  const fornecedorPorNome = new Map((existentes || []).map((f) => [f.nome.trim().toLowerCase(), f.id] as const));
  ...
  for (let i = 2; i <= planilha.rowCount; i++) {
    ...
    const payload = {
      nome,
      documento: col.documento !== undefined ? textoCelula(row.getCell(col.documento).value) || null : null,
      telefone: col.telefone !== undefined ? textoCelula(row.getCell(col.telefone).value) || null : null,
      email: col.email !== undefined ? textoCelula(row.getCell(col.email).value) || null : null,
    };

    const chave = nome.toLowerCase();
    const existenteId = fornecedorPorNome.get(chave);

    if (existenteId) {
      const { error } = await supabase.from("fornecedores").update(payload).eq("id", existenteId);
      if (error) {
        comErro++;
      ...
      const { data: novo, error } = await supabase.from("fornecedores").insert(payload).select("id").single();
      if (error || !novo) {
        comErro++;
        linhas.push({ linha: i, sku: nome, status: "erro", mensagem: error?.message ?? "Falha ao criar fornecedor." });
        continue;
      }
      fornecedorPorNome.set(chave, novo.id);
      criados++;
