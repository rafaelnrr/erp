...
export default async function DashboardPage() {
  ...
  // Pegando o nome do metadado do user (caso exista) ou caindo para o inicio do e-mail
  const userName = user?.user_metadata?.nome || user?.email?.split('@')[0] || "Usuário";

  // Dados Mockados para visualização imediata do layout
  const kpis = [
    { 
  ...
  const ultimasPropostas = [
    { id: 1, cliente: "Indústria Metalúrgica ABC", potencia: "75 kWp", valor: "R$ 215.000", status: "Aprovada" },
    { id: 2, cliente: "Supermercado Dois Irmãos", potencia: "40 kWp", valor: "R$ 112.500", status: "Pendente" },
    { id: 3, cliente: "Residência Sr. Carlos", potencia: "6.5 kWp", valor: "R$ 24.800", status: "Aprovada" },
    { id: 4, cliente: "Posto de Combustível BR", potencia: "30 kWp", valor: "R$ 88.000", status: "Recusada" },
    { id: 5, cliente: "Clínica Médica Vida", potencia: "15 kWp", valor: "R$ 48.000", status: "Pendente" },
  ];

  const statusColors: Record<string, string> = {
    "Aprovada": "bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-400",
        ...
        {kpis.map((kpi) => (
          <div key={kpi.label} className="panel p-6 flex flex-col justify-between hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
            ...
            <div className="mt-4">
              ...
              <div className={`mt-3 flex items-center text-sm font-bold ${kpi.positiva ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                {kpi.positiva ? (
            ...
            <tbody>
              {ultimasPropostas.map((prop) => (
                <tr key={prop.id}>
                  ...
                  <td className="text-slate-600 dark:text-slate-400 py-3.5">{prop.potencia}</td>
                  <td className="font-bold text-slate-900 dark:text-slate-100 py-3.5">{prop.valor}</td>
                  <td className="py-3.5">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-extrabold uppercase tracking-wider ${statusColors[prop.status]}`}>
                      {prop.status}
                    </span>
                  </td>
                </tr>
